...
const formatEventLabel = (eventType: string | undefined): string => {
...
const describeMetadataHighlights = (event: HistoryEvent): string[] => {
...
function NotebookHistoryDialog({
  ...
  const historyQuery = useQuery<HistoryQueryData>({
    ...
    queryFn: async () => {
      ...
      const events: HistoryEvent[] = Array.isArray(response.data?.events)
  ...
  const undoIsLoading = undoMutation.isPending;
