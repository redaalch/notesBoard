...
const fetchNotebookAnalytics = async ({ notebookId, range, slice }) => {
...
function LoadingState({ label = "Loading analytics…" }) {
  return (
...
function NotebookAnalyticsDialog({ notebook, open, onClose }) {
  ...
  const renderOverview = () => {
    ...
    const notesDaily = Array.isArray(data.metrics?.notesCreated?.daily)
      ? data.metrics.notesCreated.daily.map((entry) => entry.count)
      : [];
    const notesWeekly = Array.isArray(data.metrics?.notesCreated?.weekly)
      ? data.metrics.notesCreated.weekly.map((entry) => entry.count)
      : [];
    const totalNotesCreated = data.metrics?.notesCreated?.total ?? 0;
    const topTags = Array.isArray(data.metrics?.topTags)
      ? data.metrics.topTags.slice(0, 8)
      : [];
    const notebookRoles = data.metrics?.collaborators?.notebookRoles ?? {};
    const noteCollaborators =
      data.metrics?.collaborators?.noteCollaborators ?? {};
    const lastActivity = data.metrics?.lastActivity ?? null;
      ...
      <div className="space-y-6">
        <div className="grid gap-4 lg:grid-cols-3">
          ...
          <div className="rounded-2xl border border-base-300/60 bg-base-100 p-5 shadow-sm">
            ...
            <div className="mt-3 text-3xl font-bold text-base-content">
              {notesWeekly.reduce((sum, value) => sum + value, 0)}
            </div>
            <div className="mt-4 h-20">
              <Sparkline
                data={notesWeekly}
  ...
  const renderActivity = () => {
    ...
    const total = data.meta?.totals?.notesCreated ?? 0;
  ...
  const renderTags = () => {
    ...
    if (tagsQuery.isError) {
      const message = tagsQuery.error?.response?.data?.message;
      return (
  ...
  const renderCollaborators = () => {
    ...
    if (collaboratorsQuery.isError) {
      const message = collaboratorsQuery.error?.response?.data?.message;
      return (
        <ErrorState
          message={message}
  ...
  const renderSnapshots = () => {
    ...
    if (snapshotsQuery.isError) {
      const message = snapshotsQuery.error?.response?.data?.message;
      return (
        <ErrorState
          message={message}
    ...
    const snapshotCoverage = data.meta?.snapshots?.coverageRatio ?? 0;
