...
export interface NotebookPublishDialogProps {
  notebook: PublishNotebook | null;
  open: boolean;
  onClose: () => void;
  onUpdated?: (data: {
...
const buildSuggestedSlug = (name: string | undefined): string => {
...
const serializeMetadata = (metadata: Record<string, unknown> | null | undefined): string => {
  try {
...
const computeShareUrl = (slug: string): string | null => {
  if (!slug) return null;
...
function NotebookPublishDialog({
  notebook,
  open,
  onClose,
  onUpdated,
  ...
  const publishingStateQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      return response.data as PublishState;
    },
    staleTime: 30_000,
  });
  ...
  const parseMetadata = (): { value: Record<string, unknown> | null; ok: boolean } => {
