...
function NotebookInsightsDrawer({
  ...
  const [selectedSavedQueryId, setSelectedSavedQueryId] = useState(null);
  const [smartResult, setSmartResult] = useState(null);
  const [movingNotebookId, setMovingNotebookId] = useState(null);
  ...
  const smartMutation = useMutation({
    ...
    onError: (error) => {
      const message =
        error?.response?.data?.message ?? "Unable to build smart view";
      toast.error(message);
    },
  });
  ...
  const handleMoveNote = async (targetNotebookId) => {
    ...
    try {
      ...
      const message =
        error?.response?.data?.message ?? "Failed to move note to notebook";
      toast.error(message);
    } finally {
      setMovingNotebookId(null);
    }
          ...
          <div className="space-y-1">
            ...
            {note?.updatedAt ? (
              <p className="text-xs text-base-content/60">
                Last updated {formatRelativeTime(new Date(note.updatedAt))}
              </p>
            ) : null}
            {noteTags.length ? (
          ...
          <div className="rounded-2xl border border-base-300/60 bg-base-200/40 p-5">
            ...
            <div className="mt-4 flex flex-wrap items-center gap-2">
              <button
                ...
                disabled={smartMutation.isLoading}
              >
